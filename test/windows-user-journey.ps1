...
try {
    ...
    try {
        ...
        $global:LASTEXITCODE = 0
        $NextOutput = (& $InstalledAps next 2>&1 | Out-String)
        if ($LASTEXITCODE -ne 1 -or $NextOutput -notmatch "No ready work item found") {
    ...
    try {
        ...
        $global:LASTEXITCODE = 0
        & $InstalledAps audit auth --no-run *> $null
        if ($LASTEXITCODE -notin 0, 1) { throw "audit failed with exit code $LASTEXITCODE" }
