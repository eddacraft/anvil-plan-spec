...
#   $env:APS_VERSION = "v0.8.0"; & ([scriptblock]::Create((irm "..."))) --cli
#
...
function Select-ApsMode {
    ...
    switch ($choice) {
        ...
        "5" {
            $script:Mode = "setup"
            $script:SetupTarget = Read-Host "Tool (claude-code, copilot, codex, opencode, grok, antigravity, amp, gemini-cli, windsurf, roo-code, openclaw, cursor)"
...
if ($Version -match '^v?[0-9]') { $cliVersion = $Version -replace '^v', '' } else { $cliVersion = "0.8.0" }
$configBody = @"
